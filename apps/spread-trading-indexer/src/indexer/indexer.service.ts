...
export class IndexerService {
    ...
    async genIndex(): Promise<void> {
        try {
            ...
            const futuresPriceResponses = await Promise.all(
                futurePriceRequests,
            );
            const bestAskAndBidResponses = await Promise.all(
                bestAskAndBidRequests,
            );
            ...
            const entites = spotPriceRequests.map((_, index) => {
                try {
                    ...
                    const spotPricesResponseData =
                        spotPriceResponses[index].data;
                    const futuresPriceResponseData =
                        futuresPriceResponses[index].data;
            ...
            const filteredEntities = entites.filter(
                (entity) => entity !== null,
            );
